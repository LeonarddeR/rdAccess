...
class SettingsAccessorBase(AutoPropertyObject):
    ...
    @classmethod
    def createFromSettings(
        cls, driver: "RemoteDriver", settings: Iterable[driverSetting.DriverSetting]
    ):
        dct: Dict[str, Any] = {
            "__module__": __name__,
        }
        ...
        for s in settings:
            ...
            if not isinstance(
                ...
                dct[
                    f"_get_{cls._getAvailableSettingsPropertyName(s.id)}"
                ] = cls._makeGetAvailableSettings(s.id)
        log.debug("Constructed dictionary to generate new dynamic SettingsAccessor")
        return type("SettingsAccessor", (SettingsAccessorBase,), dct)(
            driver, settingNames
        )
    ...
    def _makeGetSetting(cls, setting: str):
        def _getSetting(self: SettingsAccessorBase):
            ...
            value = self.driver._attributeValueProcessor.getValue(
                attribute, fallBackToDefault=True
            )
            self.driver._queueFunctionOnMainThread(
                self.driver.requestRemoteAttribute, attribute
            )
            return value
    ...
    def _makeGetAvailableSettings(cls, setting: str):
        def _getAvailableSettings(self: SettingsAccessorBase):
            ...
            try:
                return self.driver._attributeValueProcessor.getValue(
    ...
    def __del__(self):
        ...
        try:
            for s in self._settingNames:
                self.driver._attributeValueProcessor.clearValue(
                    self._getSettingAttributeName(s)
                )
                self.driver._attributeValueProcessor.clearValue(
                    self._getAvailableSettingsAttributeName(s)
                )
        except Exception:
